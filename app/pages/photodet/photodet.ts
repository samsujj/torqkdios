...
export class PhotodetPage {
  ...
  constructor(private navCtrl: NavController,private _navParams: NavParams,private _http: Http,public modalCtrl: ModalController,public actionSheetCtrl: ActionSheetController,public toastCtrl: ToastController,sanitizer:DomSanitizationService,public alertCtrl: AlertController) {
  ...
  showComment(item){
    ...
    delcommentconfirm(item){
        ...
        var idx1 = this.itemdet.commentList.indexOf(item);
        var link = 'http://torqkd.com/user/ajs2/delcomment';
        var data = {comment_id: item.id};

        this.itemdet.commentList.splice(idx1,1);
        if(this.itemdet.commentList > 0)
            this.itemdet.commentList = parseInt(this.itemdet.commentList)-1;
